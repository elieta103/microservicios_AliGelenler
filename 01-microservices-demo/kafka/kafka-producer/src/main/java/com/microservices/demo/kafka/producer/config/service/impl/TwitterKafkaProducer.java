...
public class TwitterKafkaProducer implements KafkaProducer<Long, TwitterAvroModel> {
    ...
    public void send(String topicName, Long key, TwitterAvroModel message) {
        LOG.info("Enviando mensaje ='{}' al topico ='{}'", message, topicName);
    ...
    private void addCallback(String topicName, TwitterAvroModel message,
        ...
        kafkaResultFuture.addCallback(new ListenableFutureCallback<>() {
            ...
            public void onFailure(Throwable throwable) {
                LOG.error("Error enviando mensaje {} al topico {}", message.toString(), topicName, throwable);
            }

            @Override
            public void onSuccess(SendResult<Long, TwitterAvroModel> result) {
                    RecordMetadata metadata = result.getRecordMetadata();
                    LOG.debug("Metadatos recibidos. Topico: {}; Particion {}; Offset {}; Timestamp {}, at time {}",
                            metadata.topic(),
                            metadata.partition(),
                            metadata.offset(),
                            metadata.timestamp(),
